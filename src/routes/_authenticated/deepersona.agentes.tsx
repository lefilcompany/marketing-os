...
import {
  ArrowLeft, Brain, Sparkles, Loader2, Plus, MessageCircle, Trash2, Send, User2,
} from "lucide-react";
import { toast } from "sonner";

export const Route = createFileRoute("/_authenticated/deepersona/agentes")({
  head: () => ({ meta: [{ title: "Agentes IA — DeePersona" }] }),
...
function AgentesPage() {
  ...
  const create = useMutation({
    mutationFn: () =>
      generateAgentFromPersona({
        data: { personaId, purpose: purpose.trim() || undefined },
      }),
    onSuccess: () => {
      ...
      <div className="mx-auto max-w-6xl px-6 py-10 space-y-8">
        ...
        <header className="flex items-start gap-5">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-[10px] uppercase tracking-[0.25em] text-muted-foreground">Etapa 06 · Agentes IA</p>
            <h1 className="font-display text-3xl font-semibold tracking-tight mt-1">Agentes vivos</h1>
            <p className="text-muted-foreground mt-1 max-w-2xl">
              Cada agente encarna uma persona: entreviste, teste mensagens, valide hipóteses em conversas em tempo real.
            </p>
          </div>
          <Dialog open={open} onOpenChange={setOpen}>
            ...
            <DialogContent>
              <DialogHeader>
                ...
                <DialogDescription>
                  A IA transforma a persona em um agente conversacional.
                </DialogDescription>
              </DialogHeader>
              <div className="space-y-4">
                ...
                <div className="space-y-1.5">
                  ...
                  <Textarea
                    ...
                    placeholder="Ex.: entrevistar sobre onboarding, testar novo pricing..."
                    rows={3}
                  />
                </div>
              </div>
                ...
                <article key={a.id} className="surface-card group relative overflow-hidden p-5 space-y-3">
                  <div className="flex items-start gap-3">
                    <div
                      ...
                      style={{ background: `linear-gradient(135deg, color-mix(in oklab, ${mod.color} 65%, transparent), color-mix(in oklab, ${mod.color} 25%, transparent))` }}
                    >
        ...
        <div className="flex items-center justify-between pt-2">
          <Button asChild variant="ghost">
            <Link to="/deepersona/priorizacao"><ArrowLeft className="h-4 w-4 mr-1" /> Priorização</Link>
          </Button>
          <Button asChild variant="ghost">
            <Link to="/deepersona">Voltar ao overview</Link>
          </Button>
        </div>
      </div>

      <ChatPanel agent={chatWith} onClose={() => setChatWith(null)} />
    </div>
  );
}

function ChatPanel({ agent, onClose }: { agent: AgentRow | null; onClose: () => void }) {
  ...
  const send = useMutation({
    mutationFn: (msgs: typeof messages) =>
      chatWithAgent({ data: { agentId: agent!.id, messages: msgs } }),
  ...
  const starters = agent && Array.isArray(agent.starter_questions)
    ? (agent.starter_questions as string[])
    : [];
            ...
            <div className="space-y-2">
              ...
              {starters.slice(0, 4).map((s, i) => (
                <button
                  key={i}
                  onClick={() => submit(s)}
                  className="block w-full text-left rounded-xl border border-white/10 bg-white/[0.03] hover:bg-white/[0.06] px-3 py-2 text-sm transition"
                >
                  {s}
                </button>
              ))}
